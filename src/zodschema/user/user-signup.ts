...
export const ShopVerifyZodSchema=z.object({
  ...
  shopImage:z.any().refine((val:any)=>val!=null,{message:"1 images must be required"}),
  documentImage:z.any().refine((val:any)=>val!=null,{message:"1 images must be required"}),
  shopLocation:z.any().refine((val:{[key:string]:number})=>val?.lat && val?.lng,{message:"location must be required"}),
  yourImage:z.any().refine((val:any)=>val!=null,{message:"1 images must be required"})
})
